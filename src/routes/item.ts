...
/**
 * @swagger
 * /items/wishlist-price-changes:
 *   get:
 *     summary: Get price changes for products in user's wishlist
 *     tags: [Items]
 *     parameters:
 *       - in: query
 ...
 *       - in: query
 *         name: onlyWishlistItems
 *         schema:
 *           type: boolean
 *         description: Filter only items in user's wishlist
 *       - in: query
 *         name: userId
 *         schema:
 *           type: string
 *         description: User ID to get wishlist items for
 *     responses:
 *       200:
 *         description: List of price changes for wishlist items
 *       400:
 *         description: Invalid query parameters or missing userId
 *       500:
 *         description: Internal server error
 */
router.get("/wishlist-price-changes", (req, res) => {
  itemController.checkWishlistPriceChanges(req, res);
});
